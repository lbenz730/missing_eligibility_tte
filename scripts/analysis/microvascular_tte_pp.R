...
for(outcome in outcomes) {
  ...
  df_tmp <- 
    tibble('outcome' = case_when(outcome == 'time_combined' ~ 'Any Microvascular Event',
           ...
           'weights' = c('Unweighted', 'W^A', 'W^A x W^N', 'W^A x W^N x W^R'),
...
write_csv(df_results, glue('data/microvascular_results/file_{file_id}_pp.csv'))