...
### Set Seed
set.seed(73097)
seeds <- sample(1:1000000, max(df_files$boot_id), replace = F)
...
### Save results
if(!dir.exists('data/microvascular_bootstrap_results')) {
  dir.create('data/microvascular_bootstrap_results')
}
...
write_csv(df_results, glue('data/microvascular_bootstrap_results/bootstrap_itt_file_{file_id}.csv'))